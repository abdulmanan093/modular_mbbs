...
const Dashboard = () => {
  ...
  const breadcrumbs = [
    ...
    ...(selectedSubject
      ...
      : []),
    ...(selectedChapter
      ? [{ label: selectedChapter.name, action: () => setView("notes") }]
      : []),
  ];
            ...
            b.subjects.flatMap((s) =>
              s.chapters.flatMap((c) =>
                c.notes
                  ...
                  .map((n) => ({
                    ...n,
                    path: `Year ${y.id} → ${b.name} → ${s.name} → ${c.name}`,
                  })),
              ),
            ),
          ),
  ...
  return (
    <div className="min-h-screen bg-muted/30 flex">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-card border-r border-border transform transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <nav className="p-4 space-y-1">
          <button
            ...
            className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm font-medium text-foreground bg-secondary"
          >
          ...
          {search.length > 1 ? (
            <div className="space-y-3">
              ...
              {searchResults.map((n) => (
                <div
                  key={n.id}
                  className="bg-card rounded-lg border border-border p-4 flex items-center justify-between hover:border-primary/30 transition-colors"
                >
                  <div>
                ...
                {view === "notes" && selectedChapter && (
                  ...
                  >
                    {selectedChapter.notes.map((n) => (
                      <div
                        key={n.id}
                        className="bg-card rounded-xl border border-border p-5 flex items-center justify-between hover:border-primary/30 transition-all"
                      >
